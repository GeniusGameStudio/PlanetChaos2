    ...
    private void OnTurnChanged()
    {
        ...
        switch (currentPlayerData.Character)
        {
            case Character.Alice:
                ...
                equipBagItem = btnMainSkill.GetComponent<EquipBagItem>();
                equipBagItem.info = "回血枪，可以发射一颗有<color=#94FA4D>治愈</color>效果的子弹。\n\n<color=#94FA4D>治愈</color>效果: 可以让角色回复生命值甚至从濒死状态转为存活状态。";
                equipBagItem.equipName = "回血枪";
                break;

            case Character.Bobi:
                btnMainSkill.image.sprite = ResMgr.GetInstance().Load<Sprite>("UI/Weapon/Bobi_fireGun");
                equipBagItem = btnMainSkill.GetComponent<EquipBagItem>();
                equipBagItem.info = "火焰枪，可以发射一颗有<color=#DE1800>燃烧</color>效果的子弹。\n\n<color=#DE1800>燃烧</color>效果: 可以对角色或者物体造成燃烧伤害。";
                equipBagItem.equipName = "火焰枪";
                break;
        }
    }
